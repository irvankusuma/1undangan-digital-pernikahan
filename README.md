...
2. `prisma generate` akan otomatis berjalan saat install (`postinstall`) agar build Vercel tidak gagal.
3. Jalankan `npm run prisma:migrate`.
4. Deploy ke Vercel atau jalankan `npm run build && npm start` di VPS.
